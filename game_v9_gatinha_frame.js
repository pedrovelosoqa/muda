/*
  ...
  Rollback:
  - As versoes anteriores continuam preservadas na pasta backups/ e em game_v8_final_match.js.
  - Esta iteracao com frame recortado de gatinha.png foi salva separadamente em game_v9_gatinha_frame.js.
*/

(() => {
  const GAME_VERSION = "v9_gatinha_frame";
  ...
  class AssetBuilder {
    ...
    static ensureRuntimeTextures(scene) {
      ...
      if (scene.textures.exists("gatinha-source")) {
  ...
  class HUDController {
    ...
    reflowLives(count, width) {
      ...
      for (let index = 0; index < maxIcons; index += 1) {
        const icon = this.scene.add.image(rightX + (index * 24), 34, "gatinha-frame").setOrigin(0, 0.5);
  ...
  class MenuScene extends Phaser.Scene {
    ...
    createPlayButton() {
      ...
      this.playButtonIcon = this.add.image(-82, 0, "gatinha-frame").setDisplaySize(34, 22);
      ...
      this.footer = this.add.text(
        ...
        "Versao visual: game_v9_gatinha_frame.js | Rollbacks anteriores preservados",
        {
          fontFamily: "Trebuchet MS, Verdana, sans-serif",
          fontSize: "13px",
          color: "#dbeafe"
