/*
  ...
  Rollback:
  - As versoes anteriores continuam preservadas em game.js, game_v2_backup.js, game_v3.js, game_v4_balanced.js e game_v5_invincibility.js.
  - Esta iteracao final foi salva separadamente em game_v6_final.js.
*/

(() => {
  const GAME_VERSION = "v6_final";
  ...
  class AssetBuilder {
    ...
    static ensureRuntimeTextures(scene) {
      ...
      if (scene.textures.exists("muda-source-jpg")) {
  ...
  class HUDController {
    ...
    reflowLives(count, width) {
      ...
      for (let index = 0; index < maxIcons; index += 1) {
        const icon = this.scene.add.image(rightX + (index * 24), 34, "muda-player").setOrigin(0, 0.5);
        icon.setDisplaySize(24, 15);
  ...
  class MenuScene extends Phaser.Scene {
    ...
    createPlayButton() {
      ...
      this.playButtonIcon = this.add.image(-82, 0, "muda-player").setDisplaySize(34, 20);
      ...
      this.footer = this.add.text(
        ...
        "Versao final: game_v6_final.js | Rollbacks anteriores preservados",
        {
          fontFamily: "Trebuchet MS, Verdana, sans-serif",
          fontSize: "13px",
          color: "#dbeafe"
    ...
    startGame() {
      this.scene.start("GameScene", { phaseIndex: 0, score: 0, extraLives: 3 });
    }
  ...
  class GameScene extends Phaser.Scene {
    ...
    init(data) {
      ...
      this.extraLives = typeof data.extraLives === "number" ? data.extraLives : 3;
    }
    ...
    create() {
      ...
      this.physics.world.gravity.y = 544;
      ...
      this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
        ...
        if (this.mudaPlayer) {
          this.mudaPlayer.destroy();
          this.mudaPlayer = null;
        }
      });
    ...
    createPlayer() {
      ...
      this.mudaPlayer = new window.MudaPlayer(this, {
      ...
      this.player = this.mudaPlayer.bodySprite;
    ...
    collectPowerUp(player, powerUp) {
      ...
      this.mudaPlayer.pulseCollect();
    }

    activateInvincibility() {
      this.clearInvincibilityState(false);
      this.isInvincible = true;
      this.mudaPlayer.startInvincibilityVisual();
    ...
    clearInvincibilityState(restoreVisuals = true) {
      ...
      if (this.mudaPlayer) {
        this.mudaPlayer.stopInvincibilityVisual();
      }
    ...
    handleResize(gameSize) {
      ...
      const targetWidth = Phaser.Math.Clamp(this.layout.width * 0.1, 74, 110);
      this.mudaPlayer.setDisplayWidth(targetWidth);
      this.mudaPlayer.setPosition(this.player.x, this.player.y);
