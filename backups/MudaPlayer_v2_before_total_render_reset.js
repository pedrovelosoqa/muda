/*
  ...
  Player da Muda usando apenas o PNG puro e um rastro geometrico.
  - A gatinha usa exclusivamente a textura muda_final.
  - O arco-iris e desenhado so com Graphics e retangulos coloridos.
  - O efeito de dano usa apenas alpha no sprite.
*/
...
(() => {
  ...
  class MudaPlayer {
    constructor(scene, config) {
      ...
      this.root = scene.add.container(config.x, config.y).setDepth(30);
      this.baseSprite = scene.add.image(0, 0, "muda_final");

      this.root.add([this.baseSprite]);

      if (this.enablePhysics) {
        this.bodySprite = scene.physics.add.sprite(config.x, config.y, "muda_final");
    ...
    getTailAnchor() {
      ...
      const localX = -(this.displayWidth * 0.43);
      const localY = this.displayHeight * 0.02;
    ...
    drawRainbowTrail(time) {
      ...
      for (let stripe = 0; stripe < TRAIL_COLORS.length; stripe += 1) {
        ...
        for (let index = 0; index < this.trailHistory.length; index += 1) {
          ...
          const wave = Math.sin((index * 0.4) + (time * 0.006)) * 2.4;
