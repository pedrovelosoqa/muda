/*
  ...
  Rollback:
  - A versao anterior continua preservada em game.js e game_v2_backup.js.
  - Esta iteracao foi salva separadamente em game_v3.js.
*/
...
(() => {
  ...
  const PHASES = Array.from({ length: TOTAL_PHASES }, (_, index) => {
    ...
    return {
      ...
      spawnDelay: Math.max(860, Math.round(1640 / Math.pow(1.08, index))),
      gapRatio: Math.max(0.21, 0.34 - (index * 0.018)),
      powerUpSpeed: Math.round(145 * speedFactor),
      parallaxCloud: 18 * speedFactor,
      parallaxFar: 34 * speedFactor,
      parallaxNear: 78 * speedFactor,
  ...
  class ObstacleFactory {
    ...
    spawn(type, config) {
      ...
      const safeTop = 108;
      const safeBottom = height - floorHeight - 106;
  ...
  class GameScene extends Phaser.Scene {
    ...
    createPlayer() {
      ...
      /* A colisao e ajustada ao miolo da gatinha, nao ao retangulo completo do sprite. */
      this.player.body.setSize(this.player.displayWidth * 0.56, this.player.displayHeight * 0.60, true);
      this.player.body.setOffset(this.player.displayWidth * 0.23, this.player.displayHeight * 0.22);
    }
    ...
    flap() {
      ...
      this.player.setVelocityY(-Math.round(Math.max(320, this.layout.height * 0.43)));
