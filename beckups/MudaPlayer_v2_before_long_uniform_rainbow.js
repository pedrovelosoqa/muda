(() => {
  ...
  class MudaPlayer {
    ...
    redrawFillPatch() {
      ...
      const bodyPatch = [
        new Phaser.Geom.Point(-(width * 0.225), -(height * 0.09)),
        new Phaser.Geom.Point(-(width * 0.19), -(height * 0.145)),
        new Phaser.Geom.Point(width * 0.035, -(height * 0.145)),
        new Phaser.Geom.Point(width * 0.055, -(height * 0.105)),
        new Phaser.Geom.Point(width * 0.055, height * 0.12),
        new Phaser.Geom.Point(-(width * 0.14), height * 0.12),
        new Phaser.Geom.Point(-(width * 0.14), height * 0.055),
        new Phaser.Geom.Point(-(width * 0.185), height * 0.055),
        new Phaser.Geom.Point(-(width * 0.225), height * 0.015)
      ];
      const headPatch = [
        new Phaser.Geom.Point(width * 0.065, -(height * 0.12)),
        new Phaser.Geom.Point(width * 0.115, -(height * 0.205)),
        new Phaser.Geom.Point(width * 0.275, -(height * 0.205)),
        new Phaser.Geom.Point(width * 0.34, -(height * 0.12)),
        new Phaser.Geom.Point(width * 0.34, height * 0.115),
        new Phaser.Geom.Point(width * 0.275, height * 0.165),
        new Phaser.Geom.Point(width * 0.11, height * 0.14),
        new Phaser.Geom.Point(width * 0.065, height * 0.065)
      ];

      this.fillPatch.clear();
      this.fillPatch.fillStyle(FILL_PATCH_COLOR, 1);

      // Keep the patch well inside the outline, only covering likely transparent gaps.
      this.fillPatch.fillPoints(bodyPatch, true, true);
      this.fillPatch.fillRect(-(width * 0.04), -(height * 0.025), width * 0.08, height * 0.15);
      this.fillPatch.fillPoints(headPatch, true, true);
    }

    syncFillPatch() {
      this.fillPatch.setPosition(0, 0);
      this.fillPatch.setAlpha(this.baseSprite.alpha);
    }
