/*
  ...
  Rollback:
  - As versoes anteriores continuam preservadas na pasta backups/ e em game_v9_gatinha_frame.js.
  - Esta iteracao com asset direto gatinha_frame.png foi salva separadamente em game_v10_gatinha_asset.js.
*/

(() => {
  const GAME_VERSION = "v10_gatinha_asset";
  ...
  class AssetBuilder {
    ...
    static createFallbackTextures(scene) {
      this.generateTexture(scene, "gatinha-frame", 96, 58, (g) => {
    ...
    static ensureRuntimeTextures(scene) {
      ...
      if (scene.textures.exists("gatinha-frame-source")) {
        playerReady = true;
      }
  ...
  class HUDController {
    ...
    reflowLives(count, width) {
      ...
      for (let index = 0; index < maxIcons; index += 1) {
        const icon = this.scene.add.image(rightX + (index * 24), 34, "gatinha-frame-source").setOrigin(0, 0.5);
  ...
  class BootScene extends Phaser.Scene {
    ...
    preload() {
      ...
      this.load.image("gatinha-frame-source", "./gatinha_frame.png");
  ...
  class MenuScene extends Phaser.Scene {
    ...
    createHero() {
      ...
      this.heroPlayer = new window.MudaPlayer(this, {
        ...
        textureKey: "gatinha-frame-source",
        enablePhysics: false,
        depth: 24,
    ...
    createPlayButton() {
      ...
      this.playButtonIcon = this.add.image(-82, 0, "gatinha-frame-source").setDisplaySize(40, 25);
      ...
      this.footer = this.add.text(
        ...
        "Versao visual: game_v10_gatinha_asset.js | Rollbacks anteriores preservados",
        {
          fontFamily: "Trebuchet MS, Verdana, sans-serif",
          fontSize: "13px",
          color: "#dbeafe"
